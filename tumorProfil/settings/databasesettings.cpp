...
void DatabaseSettings::applySettings()
{
    ...
    UserInformation::instance()->setUsername(dp.userName);
    UserInformation::instance()->setPassword(dp.password);

    dp.password = QString();
    dp.writeToConfig();
}

bool DatabaseSettings::checkDatabaseConnection(DatabaseParameters parameters)
{
    qApp->setOverrideCursor(Qt::WaitCursor);
    QString databaseID(QLatin1String("TumorDbConnectionTest"));
    bool result;
    {
        QSqlDatabase testDatabase     = QSqlDatabase::addDatabase(parameters.databaseType, databaseID);
...
void DatabaseSettings::slotCheckDatabaseConnection()
{
    ...
    bool result = checkDatabaseConnection(parameters);
